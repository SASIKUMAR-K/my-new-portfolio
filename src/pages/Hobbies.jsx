    ...
    <PageWrapper>
      <div className="section-wrapper">
        ...
        <div className="hobbies-showcase">
          ...
          {/* CHESS — first */}
          <motion.div className="hobby-feature-card chess-card" initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
            ...
            <div className="hfc-content">
              ...
              <div className="hfc-traits">
                {['Strategy', 'Patience', 'Critical Thinking', 'Problem Solving', 'Foresight'].map((t, i) => (
                  <motion.span key={t} className="hfc-trait chess-trait" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.4 + i * 0.07 }}>{t}</motion.span>
                ))}
              </div>
              <motion.a href="https://www.chess.com/member/mr-sasikumar-k" target="_blank" rel="noreferrer" className="chess-profile-btn" whileHover={{ scale: 1.04, x: 4 }} whileTap={{ scale: 0.97 }}>
          ...
          {/* BADMINTON — second */}
          <motion.div className="hobby-feature-card badminton-card" initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }}>
            ...
            <div className="hfc-content">
              ...
              <div className="hfc-traits">
                {['Agility', 'Focus', 'Endurance', 'Quick Reflexes', 'Team Spirit'].map((t, i) => (
                  <motion.span key={t} className="hfc-trait" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.55 + i * 0.07 }}>{t}</motion.span>
                ))}
              </div>
            </div>
          </motion.div>
        ...
        <motion.div className="hobby-quote" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7 }}>
          <span className="mono quote-mark">"</span>
          <p>The same mindset that wins on the board and the court — wins in the codebase.</p>
          <span className="mono quote-mark">"</span>
        </motion.div>
